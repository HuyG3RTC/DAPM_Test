...
namespace WebBanHangDoreStore_2._0.Areas.Admin.Controllers
{
    ...
    public class AdminRolesController : Controller
    {
        ...
        public AdminRolesController(DorePhoneContext context, INotyfService notifyService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Edit(int id, [Bind("RoleId,RoleName,Description")] Role role)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    await _context.SaveChangesAsync();
                    _notifyService.Success("Cập nhật thành công nha");
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!RoleExists(role.RoleId))
                    {
                        _notifyService.Success("Ây da có lỗi xảy ra rồi");
                        return NotFound();
                    }
                    else
                    {
